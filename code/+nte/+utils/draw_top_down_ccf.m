function  draw_top_down_ccf(plt_data)
    ...
    %
    % Inputs [Optional]
    % plt_data - output of nte.utils.prep_data_for_top_down
    %
    % Supplying plt_data will speed up appearance of the figure.
    %
    % Example function calls:
    % td_data = nte.utils.prep_data_for_top_down
    % nte.utils.draw_top_down_ccf(td_data) %allows for faster repeat calls
    %
    % nte.utils.draw_top_down_ccf

    if nargin<1
        plt_data = nte.utils.prep_data_for_top_down;
    end 
